...
llm_config = {
    ...
    "api_key": "not_needed",  # Not used with local Ollama
...
def _enforce_response_limits(text: str) -> str:
    """Enforce approximate length and cleanliness for responses.
    Steps:
      1) Strip role/markdown headers and code fences.
      2) Join cleaned lines into a single paragraph.
      3) Limit to N sentences, ~100 words, and char cap (env overridable).
    Env overrides:
      - MAX_RESPONSE_SENTENCES (default: 5)
      - MAX_RESPONSE_CHARS (default: 900)
      - MAX_RESPONSE_WORDS (default: 100)
    """
    # Defaults with env overrides
    try:
        max_sentences = int(os.getenv("MAX_RESPONSE_SENTENCES", "5"))
    except Exception:
        max_sentences = 5
    try:
        max_chars = int(os.getenv("MAX_RESPONSE_CHARS", "900"))
    except Exception:
        max_chars = 900
    try:
        max_words = int(os.getenv("MAX_RESPONSE_WORDS", "100"))
    except Exception:
        max_words = 100
    ...
    for ln in lines:
        if not ln:
            continue
        # Drop headings and role echoes
        if ln.startswith("#"):
            continue
        # Remove role prefixes like "System:", "User:", "Assistant:", and bracketed forms like "[User]"
        ln = re.sub(r"^(?:\[)?(System|User|Assistant)(?:\])?\s*:?\s*", "", ln, flags=re.IGNORECASE).strip()
        if not ln:
            continue
        if ln.startswith("```"):
            continue
        # Drop list bullets/numbering
    ...
    # 2) Join all cleaned lines into one paragraph
    s = " ".join(cleaned)
    ...
    # 3a) Limit number of sentences
    parts = re.split(r'(?<=[.!?])(?=\s|$)', s)
    parts = [part.strip() for part in parts if part.strip()]
    if parts and max_sentences > 0 and len(parts) > max_sentences:
    ...
    if max_chars > 0 and len(s) > max_chars:
        ...
        last_space = s.rfind(' ')
        if last_space > max(0, max_chars - 30):
...
def get_llm_response(prompt: str) -> str:
    try:
        # Get the model from session or config, removing 'ollama run ' prefix if present
        model = session.get("llm_model", llm_config["model"]).replace('ollama run ', '')
        ...
        system_message = """You are a helpful AI assistant integrated into a security-focused application.
        Respond in about 100 words (3–5 short sentences). Be direct, no fluff, no marketing, no lists unless asked."""
...
"""
        ...
        # Limit tokens with a soft cap sized for ~100-word responses
        data = {
            "model": model,
            "prompt": formatted_prompt,
            "stream": False,
            "temperature": 0.4,
            "num_predict": llm_config.get("num_predict", 220),
            "top_p": 0.9,
            "top_k": 30,
            # Remove single "\n" stop to avoid premature cut; keep double newline and role markers
            "stop": ["\n\n", "###", "User:", "System:", "Assistant:"]
        }
        
        # Retry up to 2 times on timeout/transient errors
        attempts = 3
        for i in range(attempts):
            try:
                response = requests.post(
                    ...
                    headers=headers,
                    json=data,
                    timeout=90  # increase timeout to 90s to reduce false timeouts
                )
                response.raise_for_status()
                result = response.json()
                return result.get("response", "")
            except requests.exceptions.Timeout:
                print(f"LLM request timed out (attempt {i+1}/{attempts})")
                if i < attempts - 1:
                    time.sleep(1.5 * (i + 1))
                    continue
                ...
                print(f"LLM request error (attempt {i+1}/{attempts}): {e}")
                if i < attempts - 1:
                    time.sleep(1.0 * (i + 1))
                    continue
                return "I'm sorry, but I'm experiencing technical difficulties. Please try again later."
    except Exception as e:
        error_msg = f"Unexpected error in LLM response: {e}"
        print(error_msg)
        return "I'm sorry, but I'm experiencing technical difficulties. Please try again later."
...
app.config.update({
    'SESSION_COOKIE_SECURE': os.getenv('FLASK_ENV', 'development') == 'production',  # Only use secure cookies in production
...
# Force Jinja to reload templates and disable cache (dev convenience)
try:
...
def inject_csrf_token():
    ...
    return dict(csrf_token=generate_csrf(), audit_info=session.get('audit_info'))
...
# Use SecureLogger for tamper-evident DB logging
secure_logger = SecureLogger()
# Initialize ZKP_SECURITY with error handling
zkp_security = None
try:
    from zkp_security import ZKPSecurity, ZKProof
    zkp_security = ZKPSecurity()
    print("ZKP_SECURITY initialized successfully")
except Exception as e:
    print(f"ZKP_SECURITY initialization failed: {e}")
    print("Continuing without ZKP_SECURITY - security features will be limited")
...
def add_safety_guardrails(prompt: str) -> str:
    safety_prefix = (
        "You are a helpful assistant. Respond safely. "
        "Write about 100 words (3–5 concise sentences), avoid lists unless asked. "
        "Answer directly: "
    )
...
def output_filter(response: str, prompt: str = "") -> bool:
    ...
    # NEW: Context-aware filtering for conversational responses
    response_lower = response.lower()

    # Skip filtering for very short responses (likely just greetings)
    if len(response.strip()) < 50:
        return True

    # Comprehensive list of conversational indicators
    conversational_indicators = [
        "i apologize", "thank you", "you're welcome", "nice to meet you",
        "how can i help", "is there anything else", "feel free to ask",
        "don't hesitate", "let me know if", "i'm happy to help", "i'm happy to assist",
        "happy to assist", "glad to assist", "here to help", "i can help",
        "let me assist", "thanks for reaching out", "thanks for contacting",
        "i'm glad to hear", "i appreciate", "best regards", "sincerely",
        "thank you for", "i understand", "i see", "that's good",
        "i'm sorry", "excuse me", "pardon me", "my apologies",
        "it was nice", "take care", "have a great", "enjoy your",
        # Common AI response patterns that should be allowed
        "as a safety precaution", "for security reasons", "i recommend",
        "based on", "according to", "in order to", "for your protection",
        # Business/security context words that are OK in conversation
        "security features", "data privacy", "protection", "authentication",
        "suspicious activity", "fraudulent", "sensitive information", "confidential",
        # Allow these in conversational contexts (they're just explanatory)
        "transaction", "credentials", "verification", "monitoring", "audit"
    ]

    if any(indicator in response_lower for indicator in conversational_indicators):
        # For conversational responses, only check for truly dangerous patterns
        dangerous_patterns = [
            r"(?i)\bpassword\s*(is|:|=)",
            r"api[_-]?key", r"private[_-]?key",
        ...
        for pat in dangerous_patterns:
            if re.search(pat, response, re.IGNORECASE):
                print(f"Output filter: Blocked dangerous pattern in conversation: {pat}")
                return False
        return True  # Allow conversational responses

    # If the original prompt looks obviously benign, be permissive: only block truly dangerous patterns
    try:
        if prompt and _is_obviously_benign(prompt):
            dangerous_patterns = [
                r"(?i)\bpassword\s*(is|:|=)",
                r"api[_-]?key", r"private[_-]?key",
                r"\b\d{16}\b", r"\b\d{3}-\d{2}-\d{4}\b",
            ...
            for pat in dangerous_patterns:
                if re.search(pat, response, re.IGNORECASE):
                    print(f"Output filter (benign prompt): Blocked dangerous pattern: {pat}")
                    return False
            return True
    except Exception:
        pass

    # Original filtering for non-conversational responses
...
def index():
    ...
    if request.method == "POST":
        try:
            ...
            # ZKP-based prompt validation (only if ZKP_SECURITY is available)
            if zkp_security is not None:
                ...
                try:
                    ...
                    if zkp_proof is None:
                        print("Warning: ZKP proof generation returned None")
                        zkp_valid = False
                        zkp_proof = None
                    else:
                        zkp_valid = zkp_security.verify_prompt_safety_proof(zkp_proof, safety_rules)
                except Exception as zkp_err:
                    print(f"Error in ZKP validation: {zkp_err}")
                    zkp_valid = False
                    zkp_proof = None
                
                t_zkp = (datetime.now() - t0).total_seconds()

                print(f"ZKP verification result: {zkp_valid}")  # DEBUG
                # Optional SNARK policy proof (with error handling)
                t1 = datetime.now()
                try:
                ...
                except Exception as snark_err:
                    print(f"Error in SNARK validation: {snark_err}")
                    snark_valid = False
                    snark_obj = None
                
                t_snark = (datetime.now() - t1).total_seconds()
            else:
                # ZKP_SECURITY not available - use default values
                zkp_valid = True
                snark_valid = True
                zkp_proof = None
                snark_obj = None
                t_zkp = 0.0
                t_snark = 0.0
                print("ZKP_SECURITY not available - using default validation")
            ...
            # DEBUG: Print the raw LLM response before filtering
            print("=== LLM RESPONSE DEBUG ===")
            print(f"Guarded prompt: {guarded_prompt}")
            print(f"Raw LLM response: {response}")
            print(f"Response length: {len(response)} characters")
            print("========================")

            # Enforce concise response before filtering/logging to avoid long generations causing false positives
            response = _enforce_response_limits(response)

            # Output filtering
            t3 = datetime.now(); out_ok = output_filter(response, sanitized_prompt); t_out = (datetime.now()-t3).total_seconds()

            print(f"Output filter result: {out_ok}")  # DEBUG
            ...
            if not out_ok:
                ...
                return redirect(url_for("index"))
            # ZKP-based privacy-preserving logging (only if ZKP_SECURITY is available)
            if zkp_security is not None:
                try:
                    interaction_data = {
                    ...
                    print(f"ZKP log entry created successfully: {zkp_log_entry['interaction_id'][:8]}...")  # DEBUG
                except Exception as log_err:
                    print(f"Error in ZKP logging: {log_err}")  # DEBUG
                    zkp_log_entry = None
            else:
                zkp_log_entry = None
            ...
            try:
                log_id = secure_logger.log_interaction(user_id, guarded_prompt, response, risk_level=risk_level)
                print(f"Database log created successfully: {log_id}")  # DEBUG
            except Exception as log_err:
                try:
                    ...
                    pass
                log_id = None
                print(f"Database logging failed: {log_err}")  # DEBUG
                flash("Logging failed, but your message was processed.")
# Only keep the last 3 messages to prevent session cookie from getting too large
            if "chat_history" not in session:
                session["chat_history"] = []
            ...
            # Keep only the last 3 messages (1.5 exchanges)
            session["chat_history"] = session["chat_history"][-3:]
            session.modified = True
            # Calculate risk level and audit info based on available security layers
            safety_score = 0.9  # Default high safety score from other layers
            zkp_enabled = zkp_security is not None
            
            if zkp_enabled and zkp_proof is not None:
                # ZKP actually ran successfully
                safety_score = zkp_proof.metadata.get("safety_score", 0.9)
                zkp_status = "✅" if zkp_valid else "❌"
                zkp_processing_time = round(t_zkp * 1000, 2)
            elif zkp_enabled:
                # ZKP enabled but failed
                safety_score = 0.5  # Lower score due to ZKP failure
                zkp_status = "⚠️ Failed"
                zkp_processing_time = round(t_zkp * 1000, 2)
            else:
                # ZKP not available
                zkp_status = "🚫 Disabled"
                zkp_processing_time = 0.0
            ...
            audit_info = {
                ...
                'blocked_layers': {
                    'sanitizer': False,
                    'llm_self_check': None,
                    'zkp_valid': zkp_valid if zkp_enabled else None,
                    'snark_valid': snark_valid if zkp_enabled else None
                },
                'zkp_safety_score': safety_score,
                'zkp_proof_id': zkp_proof.commitment[:16] if zkp_proof else 'N/A',
                'zkp_log_id': zkp_log_entry["interaction_id"] if zkp_log_entry else 'N/A',
                'zkp_enabled': zkp_enabled,
                'zkp_status': zkp_status,
                'zkp_processing_time_ms': zkp_processing_time,
                'normalizer_version': NORMALIZER_VERSION,
                'snark_policy_id': os.getenv('SNARK_POLICY_ID', 'default'),
                'snark_score': (snark_obj or {}).get('publicSignals', {}).get('score') if snark_obj else None,
                'timings_ms': {
                    'sanitize': round(t_sanitize*1000, 2),
                    'zkp': zkp_processing_time,
                    'snark': round(t_snark*1000, 2)
                }
            }
            session['audit_info'] = audit_info
            # Safely derive short ZKP id if available
            zkp_id_short = None
            if zkp_log_entry and 'interaction_id' in zkp_log_entry:
                zkp_id_short = zkp_log_entry['interaction_id'][:8]
            
            if log_id and zkp_id_short:
                flash(f"Log ID: {log_id} | ZKP Log ID: {zkp_id_short}...")
            elif zkp_id_short:
                flash(f"ZKP Log ID: {zkp_id_short}...")
                print(f"Success: ZKP ID {zkp_id_short}... (no DB log)")  # DEBUG
            elif log_id:
                flash(f"Log ID: {log_id}")
                print(f"Success: Log ID {log_id} (no ZKP log)")  # DEBUG
            else:
                flash("Message processed (logging unavailable)")
                print("Success: message processed (no logs)")  # DEBUG
            return redirect(url_for("index"))
        except Exception as e:
            try:
                app.logger.exception(e)
                print(f"CRITICAL ERROR in request processing: {e}")  # DEBUG
...
@login_required(role="admin")
def verify():
    is_valid = secure_logger.verify_logs()
...
def log_status(log_id: int):
	status = secure_logger.get_log_status(log_id)
	if not status:
...
def log_sign():
	...
	try:
		sig = secure_logger.sign_log_as_admin(log_id, admin)
