#!/usr/bin/env python3
"""
Comprehensive evaluator for all available datasets
Processes archive data, 4k, 6k, 50k, 200k, and other data sources
"""
import os
import json
import time
...
DATASET_PATHS = {
    '4k': 'data/4kdata.json',
    '6k': 'Prompt_INJECTION_And_Benign_DATASET_COMBINED_6K_20250820_031115.jsonl',
    '50k': 'data/50kdata.json',
    '200k': 'data/200kdata.json',
    'kaggle': 'Prompt_INJECTION_And_Benign_DATASET_COMBINED_6K_20250820_031115.jsonl',
    'archive_mpdd': 'archive/MPDD.pkl'
}
...
def create_comparison_report(all_results: Dict[str, Dict[str, Any]], outdir: str) -> None:
    ...
    # Write aggregate CSV
    aggregate_path = os.path.join(outdir, 'all_datasets_comparison.csv')
    with open(aggregate_path, 'w', encoding='utf-8', newline='') as f:
        ...
        w.writerow(['dataset_name', 'collusion_detection_rate', 'false_positive_rate', 
                   'tamper_resistance', 'latency_ms', 'throughput_rpm', 'total_entries', 'sample_size'])
    ...
    with open(summary_path, 'w', encoding='utf-8') as f:
        f.write("=== Comprehensive Dataset Evaluation Summary ===\n\n")
