...
# Try to import matplotlib; fall back if unavailable
try:
	import matplotlib
	matplotlib.use('Agg')
...
TIMELOCK_DELAYS = [0, 0.005, 0.015, 0.03, 0.06]  # seconds (using small floats for fast eval)
...
def create_aggregate_visualization(all_results: Dict[str, Dict[str, Any]], outdir: str):
	...
	if not HAS_MPL:
		print("matplotlib not available; skipping aggregate visualization.")
		return
...
def main():
	...
	for size in ['4k', '6k', '50k', '120k', '200k']:
		...
		sample_n = min(len(data), 1000 if size in ['4k', '6k', '120k'] else 500)
		# Basic metrics
