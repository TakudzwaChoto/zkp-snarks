...
class ZKPSecurity:
    ...
    def generate_prompt_safety_proof(self, prompt: str, safety_rules: List[str]) -> ZKProof:
        ...
        # Canonicalize safety_rules ordering to ensure stable hashing
        try:
            canonical_rules = sorted([str(r).strip().lower() for r in safety_rules])
        except Exception:
            canonical_rules = safety_rules
        rules_hash = self._hash_data(json.dumps(canonical_rules, sort_keys=True))
    ...
    def verify_prompt_safety_proof(self, proof: ZKProof, safety_rules: List[str]) -> bool:
        try:
            ...
            now = time.time()
            delta = abs(now - proof.timestamp)
            if delta > 300:
                print(f"ZKP verify: timestamp too old. now={now}, proof.ts={proof.timestamp}, delta={delta}s > 300s")
                return False
            
            # 2) Challenge recomputation check (canonicalize rule ordering)
            try:
                canonical_rules = sorted([str(r).strip().lower() for r in safety_rules])
            except Exception:
                canonical_rules = safety_rules
            ...
            expected_challenge = self._hash_data(
                proof.commitment + rules_hash + str(proof.timestamp)
            )
            
            if proof.challenge != expected_challenge:
                print("ZKP verify: challenge mismatch.")
                print(f"  commitment: {proof.commitment}")
                print(f"  rules_hash: {rules_hash}")
                print(f"  timestamp: {proof.timestamp}")
                print(f"  got challenge: {proof.challenge}")
                print(f"  exp challenge: {expected_challenge}")
                return False
            
            # 3) Safety score threshold
            score = proof.metadata.get("safety_score", 0)
            if score < self.zkp_min_score:
                print(f"ZKP verify: safety score below threshold. score={score}, min={self.zkp_min_score}")
                return False
