...
class SecureLogger:
	def __init__(self):
		...
		# Ensure main Ed25519 keypair exists (generate if missing)
		os.makedirs("keys", exist_ok=True)
		priv_path = os.path.join("keys", "ed25519_private.pem")
		pub_path = os.path.join("keys", "ed25519_public.pem")
		if not (os.path.exists(priv_path) and os.path.exists(pub_path)):
			...
			with open(pub_path, "wb") as f:
				f.write(pub_bytes)
		# Load keypair
		self.private_key = self._load_ed25519_key(priv_path)
		self.public_key = self._load_ed25519_key(pub_path, private=False)
